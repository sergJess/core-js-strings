...
function repeatString(str, times) {
  if (times > -1) return str.repeat(times);
  return '';
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (index !== -1) {
    const indexLast = index + value.length;
    const firstStr = str.slice(0, index);
    const secondStr = str.slice(indexLast);
...
function sumOfCodes(str) {
  const isItString = typeof str === 'string' || str instanceof String;
  if (!!str && isItString) {
    let sum = 0;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.length; i += 1) {
    if (sentence[i] === ' ') {
      longest = longest.length >= index.length ? longest : index;
      index = '';
    } else {
      index += sentence[i];
    }
